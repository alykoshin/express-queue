...
const expressQueueMw = function (config) {
  ...
  let fieldValue = "default";
  if (field) {
    fieldValue = req.body[field] || req.query[field] || req.params[field];
  }

  if (!self[fieldValue]) self[fieldValue] = {};

  self[fieldValue].jobQueue = new MiniQueue(config);

  // When any task has gone to `process` state,
  // wait for `end` event of `res` object (`job.data.res`)
  // then leave the `process` state
  self[fieldValue].jobQueue.on('process', function (job, done) {
  ...
  self[fieldValue].jobQueue.on('reject', function (job) {
    debug('Rejected ' + job.data.req.path);
    rejectHandler(job.data.req, job.data.res);
  });

  self[fieldValue].queueMw = function (req, res, next) {

    const data = { req: req, res: res, next: next };
    const job = self[fieldValue].jobQueue.createJob(data);
    ...
    res.once('close', function () {
      if (job.status === 'queue') {
        ...
        self[fieldValue].jobQueue._cancelJob(job);
      }
    });
  ...
  const resultMw = function (req, res, next) {
    endMw(req, res, function () {     // Inject res.end() handler to emit 'end' event
      self[fieldValue].queueMw(req, res, next); // Use this middleware
    });
  };

  // expose queue
  resultMw.queue = self[fieldValue].jobQueue;
